# Bigpicture Performance Tests

## Generating test data

Test data can be generated and loaded into Postgres and OpenSearch with `tests/utils/generate_data.py`

For example:

```bash
uv run python -m tests.utils.generate_data --images 10000 --datasets 100
```

## Loading results

Data loading times for 1 million images:
...
## OpenSearch performance results

After generating test data, run Locust against the search API:

```bash
locust -f tests/performance/bigpicture/locustfile.py --host=http://localhost:8000
```

Then open the Locust web UI at http://localhost:8089 to configure the number of users,
spawn rate, and start the test.

## Postgres performance results

The results are for generated test data for 1O million images.

Data loading times for 10 million images:

| Number of images | Operation             | Time (seconds) |
|------------------|-----------------------|----------------|
| 10,000,000       | Loading to PostgreSQL | 911.36         |
...
-- "  ->  Sort  (cost=9937.74..9950.24 rows=100 width=16) (actual time=49.919..49.921 rows=100 loops=1)"
...
### Staining target search (GIN indexed JSON)
