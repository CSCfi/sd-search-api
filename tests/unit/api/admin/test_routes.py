...
def value_counts():
    updater = MagicMock()
    updater.refresh = AsyncMock()
    app.state.value_counts = updater
    return updater
...
def test_reload_reloads_every_cache(snomed_term_service, value_counts, client):
    resp = client.post("/admin/caches/reload", headers=_auth())
    assert resp.status_code == 204
    snomed_term_service.load.assert_called_once_with()
    value_counts.refresh.assert_called_once_with()


def test_reload_rejects_wrong_key(snomed_term_service, value_counts, client):
    resp = client.post("/admin/caches/reload", headers=_auth("wrong-key"))
    assert resp.status_code == 403


def test_reload_requires_auth_header(snomed_term_service, value_counts, client):
    resp = client.post("/admin/caches/reload")
    assert resp.status_code == 401
