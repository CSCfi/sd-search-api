...
async def ensure_index() -> None:
    ...
    if await bp_sync_service.search.indices.exists(index=BP_OPENSEARCH_INDEX):
        logging.info(
    ...
    await bp_sync_service.search.indices.create(
        index=BP_OPENSEARCH_INDEX, body=index_conf
    )
